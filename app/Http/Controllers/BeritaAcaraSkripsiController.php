...
class BeritaAcaraSkripsiController extends Controller
{
    public function index()
    {
        ...
        ->where(function($query) {
            $query->where('sidang_skripsi.dosen_penguji_1', Auth::user()->id)
                  ...
                  ->orWhere('sidang_skripsi.sekretaris', Auth::user()->id)
    ...
    public function detail($id)
    {
        $data = [
            ...
            ->select('berita_acara_skripsi.*', 'users.*', 'sidang_skripsi.*', 'bidang_ilmu.*', 'bimbingan_skripsi.*', 'bimbingan_proposal.*', 'penguji1.name as nama_penguji_1', 'penguji2.name as nama_penguji_2', 'penguji3.name as nama_penguji_3', 'sekre.name as nama_sekre')
            ->first(),
        ];
    ...
    public function store(Request $request)
    {
        ...
        $ba = new DetailBeritaAcaraSkripsi();
        $ba->users_id = Auth::user()->id;
        $ba->berita_acara_skripsi_id = $request->berita_acara_skripsi_id;
        $ba->presensi = 'hadir';
